...
func main() {
	...
	var panels = map[panel]int{panel{0, 0}: 1}
	var pos panel
	var dir, minx, maxx, miny, maxy int
	...
	painting := make([]string, maxy-miny+1)
	for y := miny; y <= maxy; y++ {
		line := make([]string, maxx-minx+1)
		for x := minx; x <= maxx; x++ {
			if panels[panel{x, y}] == 1 {
				line[x-minx] = "█"
			} else {
				line[x-minx] = " "
			}
		...
		painting[y-miny] = strings.Join(line, "")
	}

	fmt.Println(strings.Join(painting, "\n"))
}
